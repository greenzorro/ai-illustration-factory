'''
File: child_book_f_ps.py
Project: green
Created: 2025-04-24 07:05:09
Author: Victor Cheng
Email: greenzorromail@gmail.com
Description: 从UPSCALE_OUTPUT_DIR目录中复制需要PS处理的图片到PS_OUTPUT_DIR目录
'''
...
def copy_files_by_prefix():
    ...
    从PS_CSV_PATH指定的CSV文件中读取file name列作为文件名前缀，
    复制UPSCALE_OUTPUT_DIR目录中所有匹配这些前缀的文件到PS_OUTPUT_DIR目录
    """
    # 确保输出目录存在
    ...
    prefixes = []
    try:
        with open(PS_CSV_PATH, 'r', encoding='utf-8') as csvfile:
    ...
    for prefix in prefixes:
        ...
        for src_path in matched_files:
            ...
            dst_path = os.path.join(PS_OUTPUT_DIR, file_name)
    ...
    print(f"\n处理完成! 已复制 {copied_count} 个文件到 {PS_OUTPUT_DIR}")
